...
- **本 conductor の peer 名**: `hal`（注: `hal-conductor` ではなく **`hal`**）
- **本 conductor の workspace**: `.hestia/workspaces/hal/`（peer 名と一致）
- **本 conductor の 3 文書 path**:
  - `<workspace>/requirements.md` = `.hestia/workspaces/hal/requirements.md`
  - `<workspace>/design.md` = `.hestia/workspaces/hal/design.md`
  - `<workspace>/tasks.md` = `.hestia/workspaces/hal/tasks.md`

`hal-conductor/...` のような path を fs_read / fs_write してはいけません — peer 名 `hal` を一貫して使用してください。
...
```
if !agent_cli_peer_alive("hal-designer"):
  spawn-subagent --persona hal-designer --peer hal-designer
```

Phase 93 で hal-designer は常駐起動から **on-demand 起動** に変更されました（`hestia start` 直後は起動していない）。

### Step 2: 仕様作成依頼

```
agent-cli send hal-designer "{ ai-conductor からの task spec }"
...
```
for each required sub-agent (例: hal-coder-uart_rx, hal-tester など):
  spawn-subagent --persona hal-coder --peer hal-coder-uart_rx
```
