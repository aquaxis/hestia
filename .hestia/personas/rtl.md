...
```
if !agent_cli_peer_alive("rtl-designer"):
  spawn-subagent --persona rtl-designer --peer rtl-designer
```

Phase 93 で rtl-designer は常駐起動から **on-demand 起動** に変更されました（`hestia start` 直後は起動していない）。

### Step 2: 仕様作成依頼

```
agent-cli send rtl-designer "{ ai-conductor からの task spec }"
...
```
for each required sub-agent (例: rtl-coder-uart_rx, rtl-tester など):
  spawn-subagent --persona rtl-coder --peer rtl-coder-uart_rx
```
