...
**Phase 92 明確化**: 各 persona 階層が更新する 3 文書は **各エージェント専用** （per-agent / 共用ではない）であり、`.hestia/workspaces/<peer>/` 配下に独立して存在します。他エージェントの 3 文書を直接更新してはいけません — 各エージェントは自身の workspace 内 3 文書のみを更新します。
