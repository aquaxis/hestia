//! Apps Conductor message handler
...
/// Phase 126 — Upper limit on sub-agent parallelism under Apps conductor.
/// Configured via `HESTIA_PER_CONDUCTOR_MAX` (default 4). After `HESTIA_ACQUIRE_TIMEOUT_SECS`
/// (default 600) seconds, the corresponding coder spawn is skipped (deadlock detection).
...
impl AppsHandler {
    ...
    /// Phase 58 — `apps.design.v1`: Dispatch Apps design request to apps-designer.
    async fn handle_design(params: serde_json::Value) -> Result<serde_json::Value, String> {
        ...
        if !designer_alive && conductor_sdk::workspace::strict_subagent_enabled() {
            return Ok(serde_json::json!({
                ...
                "target": target,
                "note": "HESTIA_STRICT_SUBAGENT=1: apps-designer is not in the registry, halting. Check `hestia start` logs and `agent-cli list` for resident sub-agent registration status.",
            }));
        }
        if designer_alive {
            let prompt = format!(
    ...
    /// Phase 60b — `apps.dispatch_coders.v1`: Dynamic parallel spawn of apps-coder-{module}.
    async fn handle_dispatch_coders(params: serde_json::Value) -> Result<serde_json::Value, String> {
        ...
        // Phase 126 — Replace hardcoded 16 with ConductorLimiter (env-driven).
        // Phase 129 — Enforce per_conductor_max as **alive cap** (same as rtl, prefix=apps-coder-).
        ...
        if max_parallel == 0 {
            ...
            return Ok(serde_json::json!({
                ...
                "note": "apps-coder-* at per_conductor_max are alive, skipping new spawns. \
