...
impl AiHandler {
    /// Phase 59 — spec-driven 経路活性化: instruction を SpecParser で解析し
    /// 3 文書 (`AI_PRJ_REQUIREMENTS.md` / `AI_PRJ_DESIGN.md` / `AI_PRJ_TASKS.md`) の
    /// best-effort skeleton を `<workspace>/.aiprj/` に書き出す。
    /// `setup_ai` 自己実行の最小実装（Q6 ギャップ部分解消）。失敗は警告のみで
    /// 通常の workflow 構築は継続（破壊的変更を避ける）。
    fn spec_driven_emit_skeleton(instruction: &str) {
        // workspace/.aiprj/ の解決（cwd 配下、存在しない場合は no-op）
        let aiprj_dir = std::env::current_dir()
        ...
        // 既に AI_PRJ_REQUIREMENTS.md があれば setup_ai は不要（更新は update_ai 規約）
        let req_path = aiprj_dir.join("AI_PRJ_REQUIREMENTS.md");
        if req_path.exists() {
            return;
        }

        // SpecParser で REQ:/CON:/IF: プレフィックスを抽出（自然言語のみの場合は空 spec）
        let parser = SpecParser::new();
        let spec = match parser.parse(instruction) {
            Ok(s) => s,
            Err(e) => {
                tracing::warn!(error=%e, "spec_driven_emit_skeleton: parse failed, skipping");
                return;
            }
        };
        ...
        let req_md = format!(
            "# Project Requirements (auto-generated by Phase 59 spec-driven)\n\n\
             **Source**: ai.exec instruction\n\
             **Date**: {now}\n\n\
             ## Instruction\n\n```\n{instruction}\n```\n\n\
             ## Parsed Requirements ({} items)\n\n{}\n\n\
             ## Constraints ({} items)\n\n{}\n\n\
             ## Interfaces ({} items)\n\n{}\n",
            spec.requirements.len(),
            spec.requirements.iter().map(|r| format!("- {}: {}", r.id, r.text))
                .collect::<Vec<_>>().join("\n"),
            spec.constraints.len(),
            spec.constraints.iter().map(|c| format!("- {}: {}", c.id, c.text))
                .collect::<Vec<_>>().join("\n"),
            spec.interfaces.len(),
            spec.interfaces.iter().map(|i| format!("- {}: {}", i.id, i.text))
                .collect::<Vec<_>>().join("\n"),
        );
        if let Err(e) = std::fs::write(&req_path, &req_md) {
            tracing::warn!(error=%e, "spec_driven_emit_skeleton: write AI_PRJ_REQUIREMENTS.md failed");
            return;
        }

        let design_path = aiprj_dir.join("AI_PRJ_DESIGN.md");
        let design_md = format!(
            "# Project Design (auto-generated by Phase 59 spec-driven)\n\n\
             **Date**: {now}\n\n\
             ## Workflow Plan\n\n\
             Generated from `ai.exec` keyword analysis. See `AI_PRJ_TASKS.md` for the\n\
             concrete workflow_steps that AiHandler::build_workflow constructed from this\n\
             instruction. The conductor mapping follows the design spec §3.10/§4.8/§5〜§13.7\n\
             responsibility boundaries.\n"
        );
        let _ = std::fs::write(&design_path, &design_md);

        let tasks_path = aiprj_dir.join("AI_PRJ_TASKS.md");
        let tasks_md = format!(
            "# Project Tasks (auto-generated by Phase 59 spec-driven)\n\n\
             **Date**: {now}\n\n\
             Tasks are dynamically constructed by AiHandler::build_workflow from the\n\
             instruction at runtime. The structured aggregate JSON written to\n\
             `.hestia/run_log/<run-id>.json` is the authoritative task ledger for this\n\
             execution. setup_ai/update_ai/exec_job self-execution loops are pending\n\
             Phase 60+ persona-side activation.\n"
        );
        let _ = std::fs::write(&tasks_path, &tasks_md);

        tracing::info!(req=%req_path.display(), "spec_driven_emit_skeleton: 3 docs written");
    }
    ...
    async fn auto_spawn_reviewer(instruction: &str, total_steps: usize) -> bool {
        ...
        let prompt = format!(
            "[ai.exec auto-review] workflow_steps={total_steps}, instruction={:?}. Review the artifacts under <root>/{{rtl,fpga,hal,asic,pcb,apps,debug,rag}}/ and write `<root>/.aiprj/REVIEW_REPORT.md` with status: pass|fail|partial.",
    ...
    async fn handle_exec(&self, params: serde_json::Value) -> Result<serde_json::Value, String> {
        ...
        // Phase 59: spec-driven 経路活性化 — best-effort で 3 文書 skeleton を生成。
        // env override `HESTIA_DISABLE_SPEC_DRIVEN=1` で無効化可能（テスト互換性）。
        if std::env::var("HESTIA_DISABLE_SPEC_DRIVEN").as_deref() != Ok("1") {
            Self::spec_driven_emit_skeleton(&instruction);
        }
